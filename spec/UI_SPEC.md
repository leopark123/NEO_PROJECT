...
- UI 计算血氧
- UI 设置报警
- ~~UI 显示模拟/估算值~~ **例外：开发调试模式**

**开发调试模式例外**（2026-02-07 更新）：
- 当硬件不可用时（COM 端口 Blocked），可显示带明确标识的模拟值用于 UI 测试
- 必须在面板状态中明确标注 "Simulated values" 或 "开发模式"
- 模拟数据规则：
  - CH1-CH4: 58-87% 随机百分比，每秒更新
  - CH5-CH6: 显示 "Blocked"
  - CH2: 偶尔显示 "Fault"（模拟探头脱落）
- 模拟值仅用于验证 UI 渲染逻辑，不可用于临床判断
- 支持无缝切换：真实硬件接入时自动切换到真实数据流
...
|------|------|
| 播放/暂停 | 按钮切换 |
| ~~拖动 Seek~~ | ⚠️ **延后至 Phase 5** (2026-02-07 更新) |
| ~~点击跳转~~ | ⚠️ **延后至 Phase 5** (2026-02-07 更新) |
| **Seek 响应时间** | **< 200 ms** (渲染已实现，交互待实现) |
| 多流同步 | EEG/aEEG/NIRS/Video 同步跳转 |
| 同步精度 | ±100 ms |
| 审计 | SEEK (待实现) |

**当前状态**（2026-02-07）：
- SeekBar 渲染器已实现 (`SeekBarRenderer.cs`)
- 时间轴显示功能正常
- 交互功能（`TrySetSeekFromPoint`）当前禁用
- 计划在 Phase 5 高级功能中实现交互逻辑
